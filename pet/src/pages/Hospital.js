...
const Hospital = () => {
    ...
    const { setHospitalLength } = useContext(LengthContext);
    ...
    useEffect(() => {
        const fetchData = async () => {
            try {
                ...
                const HospitalRequests = data.filter(request => request.requestedRole.name === 'HOSPITAL');
                setHospitalLength(HospitalRequests.length);

                const formattedProcessingData = HospitalRequests.map(request => ({
                    id: request.id,
    ...
    const fetchApprovedData = async () => {
        try {
            ...
            const approvedData = response.data
                .filter(request => request.approved === true && request.role?.name === 'HOSPITAL')
                .map(request => ({
                    id: request.id,
                        ...
                        <div className="col-12">
                            <div className="card">
                                <div className="card-header">
                                    <h3 className="card-title" style={{ fontWeight: 'bold', fontSize: '28px' }}>Hosptial</h3>
                                </div>
